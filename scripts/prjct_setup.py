...
print_header("First-level subdirectory paths")
path = get_subdirectories(base_path=PROJECT_PATH,
                          depth=0,)
for dir_name, dir_path in path.items():
    print(f"path[\'{dir_name}\'] = {dir_path}")

# Do similar for the directory containing files from TGB
print_header("TGB paths")
tgb_path = {}
for depth in [0,1]:
    tgb_path.update(get_subdirectories(base_path=path['scripts'].joinpath('tgb'),
                                       depth=depth,)
                   )
for dir_name, dir_path in tgb_path.items():
    print(f"tgb_path[\'{dir_name}\'] = {dir_path}")