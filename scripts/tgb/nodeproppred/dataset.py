...
class NodePropPredDataset(object):
    def __init__(
        self,
        name: str,
        root: Optional[Path] = PROJ_DIR,
        meta_dict: Optional[Dict[str, Union[Path, str]]] = None,
        ...
        self.dir_name = self.meta_dict['dir_name'], a Path object  
        self.root = self.dir_name, a Path object
        self.data_dir= self.meta_dict['data_dir'], a Path object  
        self.fname = self.meta_dict['fname'], a Path object  
        self.nodefile = self.meta_dict['nodefile'], a Path object 
        """

        if meta_dict is None:
            meta_dict = {}

        if "dir_name" not in meta_dict:
            dir_name = "_".join(name.split("-"))  ## replace hyphen with underline
            self.dir_name = root.joinpath(dir_name)
            meta_dict["dir_name"] = self.dir_name
        elif isinstance(meta_dict["dir_name"],str):
            dir_name = meta_dict["dir_name"]
            self.dir_name = root.joinpath(dir_name)
            meta_dict["dir_name"] = self.dir_name
        elif isinstance(meta_dict["dir_name"],Path):
            self.dir_name = meta_dict["dir_name"]
        else:
            raise TypeError("meta_dict[\"dir_name\"] must be a str or a Path object.")

        self.root = self.dir_name

        if "data_dir" not in meta_dict:
            data_dir = "data"
            self.data_dir = self.dir_name.joinpath(data_dir)
            meta_dict["data_dir"] = self.data_dir
        elif isinstance(meta_dict["data_dir"],str):
            data_dir = meta_dict["data_dir"]
            self.data_dir = self.dir_name.joinpath(data_dir)
            meta_dict["data_dir"] = self.data_dir
        elif isinstance(meta_dict["data_dir"],Path):
            self.data_dir= meta_dict["data_dir"]
        else:
            raise TypeError("meta_dict[\"data_dir\"] must be a str or a Path object.")

        if "fname" not in meta_dict:
            fname = self.name + "_edgelist.csv"
            self.fname = self.data_dir.joinpath(fname)
            meta_dict["fname"] = self.fname
        elif isinstance(meta_dict["fname"], str):
            fname = meta_dict["fname"]
            self.fname = self.data_dir.joinpath(fname).with_suffix('').with_suffix('.csv')
            meta_dict["fname"] = self.fname
        elif isinstance(meta_dict["fname"], Path):
            self.fname = meta_dict["fname"].with_suffix('').with_suffix('.csv')
            meta_dict["fname"] = self.fname
        else:
            raise TypeError("meta_dict[\"fname\"] must be a str or a Path object.")

        if "nodefile" not in meta_dict:
            nodefile = self.name + "_node_labels.csv"
            self.nodefile = self.data_dir.joinpath(nodefile)
            meta_dict["nodefile"] = self.nodefile
        elif isinstance(meta_dict["nodefile"], str):
            nodefile = meta_dict["nodefile"]
            self.nodefile = self.data_dir.joinpath(nodefile).with_suffix('').with_suffix('.csv')
            meta_dict["nodefile"] = self.nodefile
        elif isinstance(meta_dict["nodefile"], Path):
            self.nodefile = meta_dict["nodefile"].with_suffix('').with_suffix('.csv')
            meta_dict["nodefile"] = self.nodefile
        else:
            raise TypeError("meta_dict[\"nodefile\"] must be a str or a Path object.")
        ...
        self.download()
        # check if the data directory directory exists, if not create it
        if self.data_dir.is_dir():
            print("Dataset directory is ", self.data_dir)
        else:
            raise FileNotFoundError(f"Directory not found at {self.data_dir}")
    ...
    def _version_check(self) -> None:
        ...
        if (version > 1):
            ...
            self.meta_dict["fname"] = self.data_dir.joinpath(self.name + "_edgelist_v" + str(int(version)) + ".csv")
            self.meta_dict["nodefile"] = self.data_dir.joinpath(self.name + "_node_labels_v" + str(int(version)) + ".csv")
            
            if not self.meta_dict["fname"].exists():
    ...
    def download(self) -> None:
        ...
        if self.meta_dict["fname"].exists() and self.meta_dict["nodefile"].exists():
            ...
            if inp == "y":
                ...
                if self.url is None:
                    ...
                    if self.data_dir.is_dir():
                        ...
                        self.meta_dict["data_dir"] = self.root.joinpath('data').mkdir(parents=True, exist_ok=True)
                        self.data_dir = self.meta_dict["data_dir"]

                    path_download = self.data_dir.joinpath(self.name + ".zip")
    ...
    def generate_processed_files(
        ...
        """
        OUT_DF = self.data_dir.joinpath(f"ml_{self.name}.pkl")
        OUT_NODE_DF = self.data_dir.joinpath(f"ml_{self.name}_node.pkl")
        OUT_LABEL_DF = self.data_dir.joinpath(f"ml_{self.name}_label.pkl")
        OUT_EDGE_FEAT = self.data_dir.joinpath(f"ml_{self.name}_edge.pkl")

        # * logic for large datasets, as node label file is too big to store on disc
        if self.name == "tgbn-reddit" or self.name == "tgbn-token":
            if OUT_DF.exists() and OUT_NODE_DF.exists() and OUT_EDGE_FEAT.exists():
